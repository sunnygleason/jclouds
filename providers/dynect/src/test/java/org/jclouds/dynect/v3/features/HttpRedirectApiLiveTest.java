...
public class HttpRedirectApiLiveTest extends BaseDynECTApiLiveTest {
   private final int dynectApiDelay = Integer.parseInt(System.getProperty("dynect.test.api-delay", "1000"));
   ...
   HttpRedirect redirect = HttpRedirect.builder().zone(fqdn).fqdn(redirectName).code(302).keepUri("Y").url("http://foo.com/").build();
   ...
   public void testUpdateAndPublishHttpRedirect() throws Exception {
      ...
      Job job = api().scheduleUpdate(HttpRedirect.builder().zone(fqdn).fqdn(redirectName).code(301).keepUri("N").url("http://bar.com/").build());
      assertEquals(job.getStatus(), Status.SUCCESS);
      assertEquals(api.getJob(job.getId()), job);
      Zone zone = api.getZoneApi().publish(fqdn);
      checkZone(zone);
